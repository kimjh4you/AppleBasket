	...
	// 리스트
		  public ArrayList<t_communittyVO> listCommunitty(int startRow, int endRow, String select, String search) {
		    ...
		    if (select == null || search == null) {
	            flag = false;
	            sql = "SELECT AA.* FROM (SELECT A.*, ROWNUM RNUM "
	            		+ "FROM ";
	            sql += "(SELECT T.ARTICLE_DATE, T.ARTICLE_SEQ, T.ARTICLE_TITLE, T.ARTICLE_CONTENT, T.ARTICLE_FILE, "
	            		+ "M.MB_ID as mb_id, U_NAME(M.MB_ID) AS UNAME, "
	            		+ "(SELECT COUNT(*) "
	            		+ "FROM T_COMMENT C "
	            		+ "WHERE T.ARTICLE_SEQ = C.ARTICLE_SEQ) AS countNum ";
	            sql += "FROM T_COMMUNITY T "
	            		+ "LEFT JOIN t_MEMBER M "
	            		+ "ON T.MB_ID = M.MB_ID "
	            		+ "ORDER BY T.ARTICLE_SEQ DESC ) A) AA "
